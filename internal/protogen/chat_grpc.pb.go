...
type BroadcastClient interface {
	StartStream(ctx context.Context, in *Connect, opts ...grpc.CallOption) (Broadcast_StartStreamClient, error)
...
func (c *broadcastClient) StartStream(ctx context.Context, in *Connect, opts ...grpc.CallOption) (Broadcast_StartStreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Broadcast_ServiceDesc.Streams[0], "/chat.Broadcast/StartStream", opts...)
	if err != nil {
		return nil, err
	}
	x := &broadcastStartStreamClient{stream}
...
type Broadcast_StartStreamClient interface {
	Recv() (*Message, error)
	grpc.ClientStream
}

type broadcastStartStreamClient struct {
	grpc.ClientStream
}

func (x *broadcastStartStreamClient) Recv() (*Message, error) {
	m := new(Message)
...
type BroadcastServer interface {
	StartStream(*Connect, Broadcast_StartStreamServer) error
...
func (UnimplementedBroadcastServer) StartStream(*Connect, Broadcast_StartStreamServer) error {
...
func _Broadcast_StartStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(Connect)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(BroadcastServer).StartStream(m, &broadcastStartStreamServer{stream})
}

type Broadcast_StartStreamServer interface {
	Send(*Message) error
	grpc.ServerStream
}

type broadcastStartStreamServer struct {
	grpc.ServerStream
}

func (x *broadcastStartStreamServer) Send(m *Message) error {
	...
	Streams: []grpc.StreamDesc{
		{
			...
			Handler:       _Broadcast_StartStream_Handler,
			ServerStreams: true,
		},
	},
